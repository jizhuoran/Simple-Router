...
void sr_arpcache_handle_req_sending(struct sr_instance *sr, struct sr_arpreq *req) {
  ...
  if(difftime(now, req->sent) > 1.0) {
    if(req->times_sent >= 5) {
      ...
      struct sr_packet *packet_itr = req->packets;

      while(packet_itr) {
        sr_send_icmp_err(sr, packet_itr->buf, HOST_UNREACHEABLE_TYPE, HOST_UNREACHEABLE_CODE, sr_get_interface(sr, packet_itr->iface));
...
int sr_send_arp_req(struct sr_instance *sr, uint32_t tip) {
  ...
  struct sr_if *out_interface = find_longest_prefix_match(sr, tip);
  ...
  int res = sr_send_packet(sr, packet, len, out_interface->name);
  return res;
}
